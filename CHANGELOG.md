...
### Added
- Auto-detection and auto-parsing of URLs sent directly in 1-on-1 private chats without needing `/preview` command prefix.
- Auto-welcoming message for new users when they start a 1-on-1 private chat with the bot.
- Automatic upgrading of the bot administrator's fingerprint in the SQLite configuration when it becomes available via active PGP key exchange.
- Log printing of the bot's SecureJoin QR code URL at startup to allow easy out-of-band administration bootstrapping.
