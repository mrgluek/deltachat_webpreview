...
- 🏛️ **Web Archive / KaraKeep / Archive.today / Ghostarchive Integration (`/keep <url>`):** Asynchronously save webpages across archiving services. When executed by the bot administrator with a configured [KaraKeep](https://karakeep.app/) instance, the URL is saved to KaraKeep first (with confirmation sent privately). The bot then concurrently archives the URL to the Web Archive (Wayback Machine, 120s timeout), **Archive.today** (dynamically selecting an active mirror among `archive.ph`, `archive.is`, `archive.today`, `archive.li`, `archive.vn`, `archive.md` with optional proxy routing via `ARCHIVE_TODAY_PROXY_URL`), and **Ghostarchive** (`ghostarchive.org`), seamlessly delivering snapshot links without blocking.
