...
| `OPENROUTER_API_KEY` | Optional [OpenRouter](https://openrouter.ai/keys) API key. Used as the last-resort fallback when every Gemini model fails (or as the only AI backend if `GEMINI_API_KEY` is empty). Text and images only — voice/audio summaries still require Gemini. | *(Empty)* |
| `OPENROUTER_MODELS` | Optional comma-separated OpenRouter models tried in order. `openrouter/free` routes to any currently available free model. Every AI reply names the model that produced it, e.g. `🤖 **AI** *(gemini-3.8-flash)*:`. | `openrouter/free` |
...
> [!TIP]
> **Internet Archive SPN2 API (`WAYBACK_ACCESS_KEY` & `WAYBACK_SECRET_KEY`)**:
> Unauthenticated Save Page Now requests can occasionally hit `403 Forbidden` or strict rate limits. Adding free S3 keys from [archive.org/account/s3.php](https://archive.org/account/s3.php) enables authenticated Save Page Now 2 (SPN2) API requests (`POST https://web.archive.org/save/` with `Authorization: LOW <access_key>:<secret_key>`), polling task statuses until snapshot completion with prioritized processing.
